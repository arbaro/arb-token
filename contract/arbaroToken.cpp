...
void arbaroToken::init(name name)
{
    require_auth(_self);
}

void arbaroToken::transfer(name from, name to, asset quantity, string memo)
{
    if (from == _self)
    {
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action)
{
    ...
    else if (code == receiver)
    {
        switch (action)
        {
            EOSIO_DISPATCH_HELPER(arbaroToken, (init)(testreset))
        }
    }
}
